...
import {
  AgentFunctionNodeEditor,
  DeviceCommandNodeEditor,
  DeviceStateChangedNodeEditor,
  DeviceStateIsNodeEditor,
} from './WorkflowCanvasDeviceEditors';
...
export function WorkflowCanvasInspector(props: {
  ...
  const { node, groups, providerOptions, searchProviderOptions, wecomOptions, devices, onChange, onDelete } = props;
  const parentOptions = [
  ...
  return (
    <>
      ...
      {node.type === 'device_command' ? <DeviceCommandNodeEditor node={node} devices={devices} onChange={onChange} /> : null}
      {node.type === 'agent_function' ? <AgentFunctionNodeEditor node={node} wecomOptions={wecomOptions} devices={devices} onChange={onChange} /> : null}
