...
- Gateway treats `phase` as an opaque non-empty label. It sorts `start` first, `end` last, and orders intermediate labels such as legacy `middle` or numbered samples like `sample_002` by capture time and `capture_id`.
