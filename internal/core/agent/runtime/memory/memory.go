...
func BuildPrompt(snapshot models.AgentSnapshot, sessionID string, input string) (string, map[string]any) {
...
func AppendConversation(snapshot *models.AgentSnapshot, item models.AgentConversation, now time.Time) map[string]any {
	cfg := snapshot.Settings.Memory
	if !cfg.Enabled {
