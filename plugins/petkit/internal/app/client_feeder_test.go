...
func TestApplyAccountSnapshotsEmitsFeederRecordEventOnce(t *testing.T) {
	plugin := New()
	cfg := AccountConfig{Name: "primary"}
	plugin.runtimes[accountKey(cfg)] = &accountRuntime{
		cfg:     cfg,
		client:  NewClient(cfg, defaultCompatConfig()),
	...
	plugin.applyAccountSnapshots(cfg, []deviceSnapshot{snapshot})
	plugin.applyAccountSnapshots(cfg, []deviceSnapshot{snapshot})

	var occurred int
	for len(plugin.events) > 0 {
		event := <-plugin.events
	...
	if occurred != 1 {
		t.Fatalf("expected 1 occurred event, got %d", occurred)
	}
}
