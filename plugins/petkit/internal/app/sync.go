...
func (p *Plugin) refreshDeviceIfNeeded(ctx context.Context, deviceID string) error {
	...
	p.applyAccountSnapshots(runtime.cfg, []client.DeviceSnapshot{snapshot})
	return nil
}

func (p *Plugin) applyAccountSnapshots(cfg AccountConfig, snapshots []client.DeviceSnapshot) {
	...
	if runtime == nil {
		runtime = &accountRuntime{cfg: cfg, client: client.NewClient(cfg, p.config.Compat), devices: map[string]client.DeviceSnapshot{}}
		p.runtimes[accountKey(cfg)] = runtime
	}

	next := make(map[string]client.DeviceSnapshot, len(snapshots))
...
func (p *Plugin) applySnapshot(accountName string, snapshot client.DeviceSnapshot) {
	p.mu.Lock()
	defer p.mu.Unlock()
	if old, ok := p.states[snapshot.Device.ID]; !ok {
		p.emitLocked(models.Event{
...
func (p *Plugin) snapshotForDevice(deviceID string) (client.DeviceSnapshot, bool) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	for _, runtime := range p.runtimes {
	...
	return client.DeviceSnapshot{}, false
}
