...
func commandForRequest(device *applianceRuntime, req models.CommandRequest) (string, map[string]any, map[string]any, string, error) {
	...
	case "start":
		...
		if program := client.StringFromAny(device.CurrentState["program"]); program != "" {
			params["program"] = program
		}
		if program := client.StringFromAny(req.Params["program"]); program != "" {
			params["program"] = program
		}
		if program := client.StringFromAny(req.Params["program_name"]); program != "" {
			params["program"] = program
		}
		return commandName, params, map[string]any{}, client.StringFromAny(params["program"]), nil
	case "stop":
